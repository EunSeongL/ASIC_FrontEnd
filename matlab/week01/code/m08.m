...
xcoef=[1 -1 0]; % 차분방정식의 피드 포워드(분자) 계수
ycoef=[1 -1 -2]; % 차분방정식의 피드백(분모) 계수 
yinit=[1 1]; % 초깃값 y[-2], y[-1]  

h_com_zi=iteration(xcoefzi, ycoef, yinit, impulse_sig); % 반복 대입법의 결과 
plot(n, h_com_zi, 'k'); hold on; grid on;

h_com_zs=iteration(xcoef, ycoef, yinitzs, impulse_sig); % 반복 대입법의 결과 
plot(n, h_com_zs, 'k--'); hold on; grid on;

h_com=iteration(xcoef, ycoef, yinit, impulse_sig); % 반복 대입법의 결과 
plot(n, h_com, 'r:'); hold on; grid on;


h_anal_zi = (1/3).*(-1).^n + (8/3).*2.^n; % 고전적 해법의 영입력 
plot(n, h_anal_zi, 'k-o'); hold on; grid on;

h_anal_zs = (1/3).*(-1).^n + (2/3).*2.^n; %고전적 해법의 영상태
plot(n, h_anal_zs, 'k--o'); hold on; grid on;

h_anal = (2/3).*(-1).^n + (10/3).*2.^n; % 고전적 해법의 결과 
plot(n, h_anal, 'r:*'); hold on; grid on;
xlabel('n'); legend('yzi[n]_{com}', 'yzs[n]_{com}','yc[n]_{com}', 'yzi[n]_{anal}','yzs[n]_{anal}', 'yc[n]_{anal}');
